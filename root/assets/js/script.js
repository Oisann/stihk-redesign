...
function addNewsArticle(json) {
	...
	for(var i=0; i<15; i++) {
		...
		if($('.news h1.center').text() !== "ERROR 404") stihknews.append('<tr><td>' + leadingZero(changed_date.getDay()) + '.' + leadingZero(changed_date.getMonth()) + '.' + changed_date.getFullYear() + '</td><td><h3 class="headline"><a href="/nyheter/' + article.id + '" class="normal">' + article.headline + '</a></h3>' + shorten(article.text) + '</td></tr><tr><td></td><td><a href="/nyheter/' + article.id + '" class="normal lesmer" style="font-weight: bold">Les mer...</a></td></tr>');
	}
}
