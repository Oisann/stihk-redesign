<?php
	...
	$mysql_selector = "SELECT * FROM ishaller WHERE (aktiv = 1 AND sortering != 999) ORDER BY sortering ASC;";
	...
	while ($row = mysql_fetch_array($mysql_query)) {
		$articles[intval($row['sortering'])] = array('id' => intval($row['id']), 'code' => htmlentities($row['kode']), 'short' => htmlentities($row['kortnavn']), 'name' => htmlentities($row['navn']), 'listname' => htmlentities($row['listenavn']));
	}
