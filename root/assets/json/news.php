<?php
	...
	while ($row = mysql_fetch_array($mysql_query)) {
		$articles[$counter] = array('id' => intval($row['id']), 'date' => intval($row['laget']), 'changed' => intval($row['endret']), 'type' => intval($row['type']), 'headline' => htmlentities($row['overskrift']), 'text' => addslashes(strip_tags($row['tekst'])));
		$counter++;
	}
