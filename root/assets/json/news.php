<?php
	...
	while ($row = mysql_fetch_array($mysql_query)) {
		$articles[$row['nyhet_id']] = array('date' => $row['nyhet_dato'], 'type' => $row['nyhet_type'], 'headline' => htmlentities($row['nyhet_overskrift']), 'text' => htmlentities($row['nyhet_tekst']), 'image' => htmlentities($row['nyhet_bilde']), 'hidden' => 0);
	}
